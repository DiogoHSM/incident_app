...
**Last revision**: 2026-04-29 (after drafting Plans 6/7/8)
...
| Webhook ingestion — `src/lib/ingest/**`, `src/app/api/webhooks/[sourceId]/**`, `src/app/(app)/settings/webhooks/**`, the `webhook` timeline kind body schema, `webhook_sources` / `dead_letter_webhooks` schema, `incidents.external_fingerprints` and nullable `declared_by` | spec §7 + `2026-04-29-webhooks.md` plan (drafted, not yet implemented) | Plan 6 contains an open spec deviation: §4.1 says `secret_hash` (bcrypt) but HMAC adapters need plaintext at verify time, so the plan stores AES-256-GCM ciphertext for HMAC sources and bcrypt only for Grafana bearer in a single `secret_material` jsonb column. Requires user sign-off + new env `WEBHOOK_SECRET_ENCRYPTION_KEY` before implementation. Auto-promote-severity logic and `dismissTriagingIncident` (extends `StatusChangeBody.dismissed`) live here too. |
| Public status page — `src/lib/status/**`, `src/lib/db/queries/status-snapshot.ts`, `src/app/(public)/status/**`, `status_snapshots` schema | spec §3.3 + §5.2 + §6.5 + §8.4 + `2026-04-29-status-page.md` plan (drafted, not yet implemented) | Adds `status_update_published` timeline kind (only kind shown on `/status`). Two NOTIFY channels: `incident_updates` (existing, war-room SSE) + `status_snapshot_updated` (new, forward-looking hook for Plan 9 ISR busting; v1 just relies on `revalidate=15`). `changeIncidentStatus` / `declareIncident` / `dismissTriagingIncident` MUST also call `recomputeAllSnapshotsForTeam(tx, teamId)` inside their tx. Public routes are unauthenticated — middleware must allow `/status/**` and `/api/...` exceptions handled separately. |
| Metrics & dashboard — `src/lib/metrics/**`, `src/lib/db/queries/metrics.ts`, `src/lib/db/queries/dashboard.ts`, `src/app/(app)/dashboard/**`, `src/app/(app)/metrics/**`, `recharts` dep | spec §5.1 + §6.2 + build seq #10 + `2026-04-29-metrics.md` plan (drafted, not yet implemented) | Read-only on schema (no migrations). MTTR excludes dismissed incidents; MTTA only counted for webhook-declared incidents (declared_by IS NULL); plan defends against Plans 6/7 being unmerged. recharts 2.x is the chart lib; heatmap is hand-rolled HTML for a11y. Aggregators are pure + unit-tested separately from queries. Beware the `recharts dataMax string-coercion` global memory entry. |
